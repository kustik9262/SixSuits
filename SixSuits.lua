...
SMODS.Back {
    ...
    pos = { x = 3, y = 0 },
    unlocked = false,
    unlock_condition = { spectrums = 5 },
    check_for_unlock = function(self, args)
        if args.type == 'win_deck' and (G.GAME.hands[base_spectrum.key].played + G.GAME.hands[str_spectrum.key].played + G.GAME.hands[spec_house.key].played + G.GAME.hands[spec_five.key].played) >= self.unlock_condition.spectrums then
            return true
        end
    end,
